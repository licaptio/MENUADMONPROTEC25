...
document.querySelector("#btnActualizarRfc")
  .addEventListener("click", cargarRfcAutorizados);
  ...
  for (let index = 0; index < archivosSeleccionados.length; index++) {
    ...
    try {
      ...
      try {
        ...
        if (await existeEntradaFirebase(cfdi.uuid)) {
          actualizarProceso("firebase", index, "warn", "Ya existía", detalle);
          log(`⚠️ Firebase ya contenía ${cfdi.uuid}.`);
          continue;
        }

        await guardarEntradaFirebase(cfdi.uuid, cfdi.datosInsert);
        resumen.firebase++;
        actualizarResumen(resumen);
