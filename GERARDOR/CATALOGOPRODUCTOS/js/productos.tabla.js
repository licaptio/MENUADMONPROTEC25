...
const MAX_FILAS_DOM = 300;

function totalFiltradosReal(){
  return Array.isArray(state.filtrados) ? state.filtrados.length : 0;
}

export function renderTabla(){
  const total = state.productos.length;
  const totalFiltrado = totalFiltradosReal();
  const visibles = state.filtrados.slice(0, MAX_FILAS_DOM);

  meta.textContent = `Total: ${total} | Encontrados: ${totalFiltrado} | En pantalla: ${visibles.length}`;
  ...
  if (totalFiltrado > MAX_FILAS_DOM) {
    tbody.insertAdjacentHTML("beforeend", `
      <tr>
        <td colspan="9" style="text-align:center;font-weight:800;color:#475569;background:#f8fafc;">
          Se muestran ${MAX_FILAS_DOM} de ${totalFiltrado}. Escribe más específico para reducir resultados.
        </td>
      </tr>
    `);
  }
...
export function reaplicarVistaActual(){
  ...
  if (!texto) {
    ...
    renderTabla();
    meta.textContent = `Catálogo local | Total: ${state.productos.length} | Encontrados: ${state.filtrados.length} | En pantalla: ${Math.min(state.filtrados.length, MAX_FILAS_DOM)}`;
    return;
  }
