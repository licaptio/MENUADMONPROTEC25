...
bitacora_cambios_cantidad: obtenerBitacoraCambiosCantidad()
  ...
  try {
    ...
    const ventaRecuperadaRaw = localStorage.getItem("ventaEnEsperaRecuperadaPOS");
    let ventaRecuperada = null;

    try {
      ventaRecuperada = ventaRecuperadaRaw ? JSON.parse(ventaRecuperadaRaw) : null;
    } catch (e) {
      ventaRecuperada = null;
    }

    const resultadoLocal = ventaRecuperada?.documento_id
      ? prepararVentaRecuperadaCobradaLocal(ventaRecuperada.documento_id, ventaBase, ventaRecuperada)
      : prepararVentaCobradaLocal(ventaBase);
    ...
    localStorage.setItem("ultimaVentaPOS", JSON.stringify({
