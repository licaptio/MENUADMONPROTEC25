...
export async function procesarTexto(texto) {
  ...
  // 🔥 CAMPO PRINCIPAL = SOLO CÓDIGO EXACTO
  let producto = resolverProductoPorCodigo(q);

  // 🔁 Si no existe directo, buscar equivalencia remota
  if (!producto) {
  ...
  // ❌ No usar buscarLocal aquí porque encuentra parciales
  toast("Código no existe");
  limpiarBusqueda(input, resultados);
}
