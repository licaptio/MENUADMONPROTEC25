...
import { renderPantallaPOS } from "../pos/pantallaPOS.js";
...
function restaurarSesion() {
  ...
  try {
    ...
    console.log("Sesión restaurada:", USUARIO_LOGUEADO.nombre || USUARIO_LOGUEADO.usuario);
...
async function entrarAlPOS() {
    ...
    posApp.innerHTML = `
      <div class="loading-screen">
        <div class="loading-card">
          ...
          <p id="loadingMsg">Descargando catálogo...</p>
        </div>
      </div>
    `;
  }
  ...
  try {
    ...
    if (loadingMsg) {
      loadingMsg.textContent = `Catálogo cargado: ${catalogo.length} productos`;
    }
    ...
    const loadingMsg = document.getElementById("loadingMsg");
    if (loadingMsg) {
      loadingMsg.textContent = "Error cargando catálogo";
    }
  }
}

function mostrarPantallaPOS() {
  renderPantallaPOS({
