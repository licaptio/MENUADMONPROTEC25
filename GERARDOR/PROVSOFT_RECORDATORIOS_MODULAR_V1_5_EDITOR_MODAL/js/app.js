import {
  ...
  revivirRecordatorio,
  establecerPrioridadRecordatorio
} from "./firestore.js";
...
async function cambiarPrioridad(id, prioritario) {
  try {
    await establecerPrioridadRecordatorio(id, prioritario);
    mostrarMensaje(prioritario
      ? "Marcado como prioritario. Permanecerá destacado hasta que lo desmarques o atiendas."
      : "Prioridad retirada.", "success");
    await cargarListado("PENDIENTE");
  } catch (error) {
    console.error("app.js | Error al cambiar prioridad:", error);
    const mensaje = error.code === "prioridad/limite-7"
      ? error.message
      : `No se pudo cambiar la prioridad: ${error.message}`;
...
async function cargarListado(estado = obtenerVistaActual()) {
  ...
  try {
    ...
    renderizarRecordatorios(recordatorios, estado, cambiarPrioridad);
