...
let onCambiarPrioridadActual = null;
...
function obtenerRecordatoriosVisibles(recordatorios) {
  const selector = document.getElementById("filtroFechaMovil");
  if (!esVistaMovil() || !selector?.value) return recordatorios;

  return recordatorios.filter(r => {
    const f = r.fecha_programada?.toDate?.();
    return f && fechaLocalISO(f) === selector.value;
  });
}

function inicializarFiltroFechaMovil() {
  const selector = document.getElementById("filtroFechaMovil");
  if (!selector || selector.dataset.inicializado === "1") return;

  selector.dataset.inicializado = "1";
  selector.value = fechaLocalISO(new Date());
  selector.addEventListener("change", () => renderizarRecordatorios(recordatoriosActuales, vistaActual));
}

export function renderizarRecordatorios(recordatorios, estado = "PENDIENTE", onCambiarPrioridad = null) {
  if (typeof onCambiarPrioridad === "function") onCambiarPrioridadActual = onCambiarPrioridad;
  recordatoriosActuales = recordatorios;
  vistaActual = estado;
  inicializarFiltroFechaMovil();
  const visibles = obtenerRecordatoriosVisibles(recordatorios);

  const esAtendidos = estado === "ATENDIDO";
  tituloListado.textContent = esAtendidos ? "Recordatorios atendidos" : "Recordatorios pendientes";
  tabPendientes.classList.toggle("active", !esAtendidos);
  tabAtendidos.classList.toggle("active", esAtendidos);

  if (!visibles.length) {
    resumen.innerHTML = `<span class="badge">0 ${esAtendidos ? "atendidos" : "pendientes"}</span>`;
    const selector = document.getElementById("filtroFechaMovil");
    const sufijoFecha = esVistaMovil() && selector?.value ? ` para el ${selector.value.split("-").reverse().join("/")}` : "";
    listaRecordatorios.innerHTML = `<div class="empty">No hay recordatorios ${esAtendidos ? "atendidos" : "pendientes"}${sufijoFecha}.</div>`;
    return;
  }

  const ahora = new Date();
  let vencidos = 0, hoy = 0, proximos = 0;
  const totalPrioritarios = esAtendidos ? 0 : recordatorios.filter(r => r.prioritario === true).length;

  listaRecordatorios.innerHTML = visibles.map(r => {
    const f = r.fecha_programada?.toDate?.() || new Date();
    const misma = f.toDateString() === ahora.toDateString();
    let clase = esAtendidos ? "attended" : "future";
    ...
    const nota = (r.anotacion_extra || "").trim();

    const fechaTexto = f.toLocaleDateString("es-MX", { day: "2-digit", month: "2-digit", year: "numeric" });
    const horaTexto = f.toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit", hour12: false });
        ...
        <article class="reminder-card ${clase}" data-id="${escAttr(r.id)}" tabindex="0">
          ...
          <div class="reminder-preview ${imagenes.length ? "" : "no-images"}">
            ...
            <div class="reminder-texts">
              ...
              ${nota ? `<p class="reminder-note-summary">Nota: ${esc(nota)}</p>` : ""}
            </div>
          </div>
        </article>`;
    }

    const esPrioritario = r.prioritario === true;
    const clasePrioridad = esPrioritario ? " priority-blink" : "";

    return `
      <article class="reminder-card ${clase}${clasePrioridad}" data-id="${escAttr(r.id)}" tabindex="0">
        <div class="reminder-row-grid">
          <div class="reminder-general">
            <div class="reminder-head">
              <div class="reminder-head-main">
                <h3 class="reminder-title">${esc(r.titulo || "Recordatorio")}</h3>
                <p class="reminder-meta">${esc(r.estado)}</p>
              </div>
              <div class="reminder-head-actions">
                <label class="priority-check" title="Marcar como uno de los 7 prioritarios">
                  <input type="checkbox" class="priority-checkbox" data-priority-id="${escAttr(r.id)}" ${esPrioritario ? "checked" : ""}>
          ...
          <div class="reminder-date-cell"><span class="reminder-cell-label">Fecha</span><strong>${fechaTexto}</strong></div>
          <div class="reminder-time-cell"><span class="reminder-cell-label">Hora</span><strong>${horaTexto}</strong></div>
        </div>
      </article>`;
  }).join("");

  if (esAtendidos) {
    resumen.innerHTML = `<span class="badge">${visibles.length} atendidos</span><span class="badge">Se pueden editar o revivir</span>`;
  } else {
    resumen.innerHTML = `
      <span class="badge">${visibles.length} pendientes</span>
      <span class="badge priority-counter">${totalPrioritarios}/7 prioritarios</span>
      <span class="badge">${vencidos} vencidos</span>
      <span class="badge">${hoy} para hoy</span>
      <span class="badge">${proximos} próximos</span>`;
  }

  document.querySelectorAll(".priority-checkbox").forEach(check => {
    check.addEventListener("click", e => e.stopPropagation());
    check.addEventListener("keydown", e => e.stopPropagation());
    check.addEventListener("change", async e => {
      e.stopPropagation();
      if (typeof onCambiarPrioridadActual !== "function") return;
      const input = e.currentTarget;
      const valorAnterior = !input.checked;
      input.disabled = true;
      try {
        await onCambiarPrioridadActual(input.dataset.priorityId, input.checked);
      } catch (error) {
...
export function inicializarTabs(onCambioVista) {
  tabPendientes.addEventListener("click", () => onCambioVista("PENDIENTE"));
  tabAtendidos.addEventListener("click", () => onCambioVista("ATENDIDO"));
}
...
export function inicializarEditor() {
  [editorFecha, editorHora, editorContenido, editorAnotacionExtra].forEach(elemento => {
...
function abrirEditor(id) {
  ...
  editorAnotacionExtra.value = r.anotacion_extra || "";
  btnAtendido.textContent = r.estado === "ATENDIDO" ? "Revivir recordatorio" : "Marcar atendido";
  btnAtendido.classList.toggle("revive", r.estado === "ATENDIDO");
...
export function obtenerDatosEditor() {
  ...
  return {
    ...
    anotacion_extra: editorAnotacionExtra.value,
    estado: editorRecordatorio?.estado || "PENDIENTE",
